...
class GTmetrix_For_WordPress {
    ...
    public function __construct() {
        ...
        define( 'GFW_GA_CAMPAIGN', '?utm_source=wordpress&utm_medium=GTmetrix-v' . GFW_VERSION . '&utm_campaign=' . rawurlencode( get_option( 'blogname' ) ) );
    }
    ...
    public function add_to_toolbar( $wp_admin_bar ) {
        ...
        if ( GFW_AUTHORIZED && ! is_admin() && current_user_can( 'access_gtmetrix' ) && isset( $options['toolbar_link'] ) && $options['toolbar_link'] ) {
            ...
            $wp_admin_bar->add_menu( array(
                ...
                'href'   => GFW_TESTS . '&url=' . isset( $_SERVER['REQUEST_URI'] ) ? esc_html( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : '',
            ) );
        }
    }
    ...
    public function system_check() {
        ...
        if ( is_plugin_active( $plugin ) ) {
            ...
            if ( isset( $message ) ) {
                ...
                wp_die( esc_html( $message ) . 'Deactivating Plugin.</p><p>Back to <a href="' . esc_html( admin_url() ) . '">WordPress admin</a>.</p>' );
            }
        }
    }
...
HERE;
    ...
    public function admin_notices() {
        if ( ! GFW_AUTHORIZED ) {
            echo $this->set_notice( '<strong>GTmetrix for WordPress is almost ready.</strong> You must <a href="' . esc_html( GFW_SETTINGS ) . '">enter your GTmetrix API key</a> for it to work.' );
        }
    ...
    public function set_api_username() {
        $options = get_option( 'gfw_options' );
        echo '<input type="text" name="gfw_options[api_username]" id="api_username" value="' . ( isset( $options['api_username'] ) ? esc_html( $options['api_username'] ) : '' ) . '" />';
    }

    public function set_api_key() {
        $options = get_option( 'gfw_options' );
        echo '<input type="text" name="gfw_options[api_key]" id="api_key" value="' . ( isset( $options['api_key'] ) ? esc_html( $options['api_key'] ) : '' ) . '" />';
    }
    ...
    public function set_default_location() {
        ...
        foreach ( $options['locations'] as $location ) {
            echo '<option value="' . esc_html( $location['id'] ) . '" ' . selected( $options['default_location'], $location['id'], false ) . '>' . esc_html( $location['name'] ) . '</option>';
        }
    ...
    public function set_notifications_email() {
        ...
        foreach ( array( 'api_username' => 'GTmetrix email (' . $options['api_username'] . ')', 'admin_email' => 'Admin email (' . get_option( 'admin_email' ) . ')' ) as $key => $value ) {
            echo '<option value="' . esc_html( $key ) . '" ' . selected( $options['notifications_email'], $key, false ) . '>' . esc_html( $value ) . '</option>';
        }
        echo '</select></p>';
    }
    ...
    public function set_front_url() {
        ...
        foreach ( array( 'wp' => 'WordPress Address (' . site_url() . ')', 'site' => 'Site Address (' . home_url() . ')' ) as $key => $value ) {
            echo '<option value="' . esc_html( $key ) . '" ' . selected( $options['front_url'], $key, false ) . '>' . esc_html( $value ) . '</option>';
        }
        echo '</select></p>';
    }
